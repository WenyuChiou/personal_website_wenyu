...
/**
 * Open drawer with category filter - Global function for category buttons
 */
window.openDrawerWithFilter = function (category) {
  const drawer = document.getElementById('projectDrawer');
  const backdrop = document.getElementById('drawerBackdrop');
  const drawerTitle = document.getElementById('drawerTitle');
  const drawerContent = document.getElementById('drawerContent');
  ...
  if (!drawer || !data || !data.projects) {
    console.warn('[Drawer] Cannot open - missing elements or data');
    return;
  }

  // Category labels and colors
  const categoryConfig = {
    research: { label: { en: 'Research', zh: '研究' }, color: '#3b82f6', icon: '🔬' },
    side: { label: { en: 'Side Project', zh: '副專案' }, color: '#8b5cf6', icon: '💻' },
    intern: { label: { en: 'Internship', zh: '實習' }, color: '#10b981', icon: '🏢' }
  };

  // Set title based on category
  const titles = {
    all: { en: 'All Projects', zh: '所有專案' },
    research: { en: 'Research Projects', zh: '研究專案' },
    side: { en: 'Side Projects', zh: '副專案' },
    intern: { en: 'Internships', zh: '實習經歷' }
  };
  drawerTitle.textContent = titles[category]?.[lang] || titles.all[lang];

  // Filter projects
  let projects = data.projects.items;
  if (category !== 'all') {
    projects = projects.filter(p => {
      // Check category field or tags for intern
      if (category === 'intern') {
        return p.tags?.some(t => t.toLowerCase().includes('intern'));
      }
      return p.category === category;
    });
  }

  // Render cards with category badge
  drawerContent.innerHTML = projects.map(project => {
    // Determine category for badge
    let projCategory = project.category || 'research';
    if (project.tags?.some(t => t.toLowerCase().includes('intern'))) {
      projCategory = 'intern';
    }
    const catConfig = categoryConfig[projCategory] || categoryConfig.research;

    return `
      <div class="drawer-project-card reveal">
        <span class="category-badge" style="background: ${catConfig.color};">
          ${catConfig.icon} ${catConfig.label[lang]}
        </span>
        ${project.image ? `
          <div class="project-image">
            <img src="${project.image}" alt="${project.name[lang]}" loading="lazy">
        ...
        <div class="project-info">
          ...
          <div class="project-links">
            ...
            ${project.links?.poster ? `<a href="${project.links.poster}" target="_blank">📄 Poster</a>` : ''}
            ${project.links?.paper ? `<a href="${project.links.paper}" target="_blank">📑 Paper</a>` : ''}
            ${project.links?.report ? `<a href="${project.links.report}" target="_blank">📋 Report</a>` : ''}
          </div>
        </div>
      </div>
    `;
